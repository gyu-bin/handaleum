...
| 공유 시트: iOS=`Share.share({url})`, Android=`expo-sharing` (파일 Intent). **탑레벨 import 금지** — 네이티브 미링크 바이너리에 OTA하면 `/cards` 배럴이 미리보기까지 평가되며 크래시. require + 앨범 폴백 | Android RN Share는 이미지 첨부 불가 | 사용자 요청 2026-08-27. 2026-08-28 OTA 크래시 | 2026-08-28 |
