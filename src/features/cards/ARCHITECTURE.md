...
| 연속 촬영 일수 | 파생 + kv `photoStreakEpoch`/`photoStreakDays` | 앨범 GPS 촬영일. 최고는 날짜 집합에서 계산 |
...
| 공유 시트: iOS=`Share.share({url})`, Android=`expo-sharing` (파일 Intent) | Android RN Share는 이미지 첨부 불가 | 사용자 요청 2026-08-27. 앨범 저장 우회 제거 | 2026-08-27 |
...
| 카드 만들기 sticky 프리뷰 = 인스타식 latch. 그리드 내리면 **완전 숨김**(minRatio 0). 사진 스크롤은 다시 안 펼침. 시트 헤더를 내려야 카드가 보임 | 축소만 남기기 / 맨 위에서 자동 펼침 | 사용자: 작아지는 게 아니라 사라지고, 헤더를 내려야 보임 | 2026-08-21 |
| 회고 `찍은 사진` 집계 = `displayPhotos.length` (GPS+no-GPS). 장소·journey·핀은 `photos`만 | GPS-only 카운트 | no-GPS 정책. 카드 photoRefs schema 불변 | 2026-09-17 |
...
| 내 회고 날 = GPS 사진 연속 일수 (이 빌드 첫 실행일부터). 위치/날 오른쪽. 10·20·30일 불꽃. 10일 단위는 앱 진입 팝업. 주완·만월 이스터에그는 하지 않음 | 달력 도장 / 홈 노출 | 사용자 2026-08-21 빼기로 확정 | 2026-08-21 |
| 내 회고 보드·별명 시트 = 셸 다크 토큰 (빈 칸·캡션·레일·요일). 카드 종이 템플릿은 유지 | 보드만 크림 고정 | 사용자 2026-08-21 | 2026-08-21 |
| 요약 탭 = 대표사진·핵심숫자·TOP4 장소 카드·기록 CTA. 순간 섹션·메모 탭 제거. 장소 grouping=visitPlaces identity, 장수순 | 인사이트 대시보드 / 새 clustering | 사용자 디자인 가이드 | 2026-09-13 |
| 회고 3탭 라벨 = 요약 / 장소 / **월** (달력 day 보드). 예전 `사진`은 오해 소지 | 사진 | 사용자 2026-09-15 | 2026-09-15 |
| 요약 히어로 = kv `monthCover:{YYYY-MM}` 사용자 월 커버. 앨범에 없으면 `monthPhotos[0]` fallback. 뷰어 “대표로 쓰기” | 월 피커·핀과 공유 / 고정 첫 장 | 사용자 Plan Summary Hero Cover | 2026-09-14 |

## 경계

- 이 feature가 의존하는 것: photos (photoRefSchema, monthKeySchema, useMonthlyPhotos, useCurrentMonth, useMonthJourney), lib/storage(MMKV), react-native-view-shot, expo-sharing(Android 카드 공유), shared/constants
- 이 feature에 의존하는 것: 없음

## 범위 제외

- TODO(출시 전): GPS 없는 사진을 카드 선택·저장에 지원하려면 `recapCard.photoRefs` 저장 schema와 기존 저장 카드의 migration을 함께 설계한다. 이번 작업에서는 기존 `PhotoRef` 계약을 유지한다.
- 카드 편집(저장 후 수정) — 삭제 후 재생성으로 대체, 필요 시 재검토
- 주완·만월 이스터에그 도장 (일~토 한 줄 / 그달 만착). 연속 촬영 문구·불꽃은 유지
