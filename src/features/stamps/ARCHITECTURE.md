...
| 라이브 적립 전에 이번 달 사진을 동 사진 인덱스에 merge. 레드닷은 merge 이후. 주간 GPS 스냅샷에 오늘 사진이 없어도 팝업이 비지 않음 | 스냅샷만 / 레드닷 먼저 | 사용자: NEW 찍혔는데 동 사진 없음 | 2026-08-30 |
| 동 사진 인덱스: GPS 스냅샷은 **documentDirectory** (캐시 퍼지와 분리). 빈 재빌드는 기존 인덱스를 덮지 않음. in-flight PIP는 generation으로 폐기 | cacheDirectory + 마지막 빌드 무조건 commit | 도장은 있는데 동 사진 없음 | 2026-09-06 |
| 콜드스타트 동 PIP는 32장마다 UI 양보 (`forEachPipChunk`). 스냅샷 인덱스·이번 달 적립 모두. 도장/레드닷은 조금 늦을 수 있음 | 한 틱에 전량 | 홈·발도장 인덱스 빌드가 한 틱에 멈추던 것 | 2026-08-31 |
| 발도장 진입 시 새 동 그리드 일괄 슬램 제거. 팝업만 축하하고, PIP 인덱스는 팝업·unseen 끝난 뒤 afterInteractions. 구 이동은 팝업 닫힌 다음 | 진입 즉시 L1+전원 슬램+PIP | 오랜만 + 새 동 많을 때 발도장 버벅임 | 2026-08-31 |
...
| 발도장 루트 UI = **여행 스탬프북** (시·도 TravelStamp 3열 그리드 · 미방문 outline · 지도는 헤더 모달). 시·도 탭 → 히어로+시군구 도장판+순간. 해외 탭 없음 | 지도 위 컬러 원 / 국내·해외 세그먼트 | 사용자 시안: 도장 수집이 본체 | 2026-09-13 |
| 발도장 seal = **Slate Navy / Muted Blue** (`stampInk*`) 원형 스탬프 + 빈 outline. 진행률은 micro 보조. 획득 연출은 쿵+잉크번짐만 | terracotta 도장 / 숫자 카드 지배 | 도장판·여권 감성 개편 | 2026-09-13 |
...
| 방문 지도 = 헤더 아이콘 → 팝업. 한반도 + 동 방울 + **핀치 줌·팬**(모달 닫으면 리셋) | 고정 배율 | 확대 피드백 | 2026-08-16 |
| 동 사진 대량 그리드: `removeClippedSubviews` off, window↑, Android URI **drop-oldest**+재시도 | clip/window 좁음 / drop-newest | 2000장+ 하단 미로딩 | 2026-08-16 |
...
| 발도장에서 **방문 지도 제거** (헤더 맵 버튼·모달). 한달음 톤에 전국 누적 지도가 안 맞음. 스탬프북만 | 방문 지도 보조 유지 | 사용자 | 2026-09-13 |
| 도장 비주얼 = 시·도 17 PNG + 구·동은 프레임 SVG + **심볼 PNG 12** (`assets/stamps/symbols`). mapping/hash. 구·동별 PNG 금지. | 구 251 PNG / 못생긴 선 글리프 | 시 퀄리티 + 확장 가능한 심볼 라이브러리 | 2026-09-14 |
