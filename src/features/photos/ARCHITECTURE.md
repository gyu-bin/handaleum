...
| 방문 마커: **네이버 `image.httpUri`(앱 캐시 JPEG)**. 커스텀 React children 스냅샷은 실기기에서 Image 미페인트. `expo-image-manipulator`로 export, localUri 우선 | 커스텀 View + remount / ph:// 직접 | TestFlight 빈 핀 재발 → 네이티브 image 경로로 전환 | 2026-07-28 |
