...
| 지도 **자치구 경계**: 특별시·광역시만 `districts.json` (서울 25 + 부산·대구·인천·광주·대전·울산). region/local에서 선+이름, 라벨은 greedy 충돌 제거 | 전국 시군구 / 경계 없음(텍스트만) | 서울·광역시 내부가 비어 보이던 UX. 전국 시군구는 확대 시 선이 깨져 보여 계속 제외 | 2026-07-24 |
...
- 이 feature가 의존하는 것: expo-media-library, expo-location, react-native-svg, react-native-zoom-toolkit, react-native-gesture-handler, react-native-reanimated, assets/geo/korea.json, assets/geo/dong-gu.json, assets/geo/districts.json, lib/storage, shared/constants
...
- MapLibre / 실제 타일 지도
- 전국 시군구·일반구 경계 (특별시·광역시 자치구만 `districts.json`)
- 몰아보기 자동 재생 (1차 릴리즈)
- 카드 플로우 전면 개편
