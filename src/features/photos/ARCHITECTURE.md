...
| 월중 추억 알림 | 로컬 스케줄 (`memoryReminder`) — OS 알림 권한만. 설정 토글 없음 | 월 1통. 탭 시 content month + 몰아보기(해당 asset) |
...
| 몰아보기 시트 탭 = **크게 보기**(좌우 스와이프). 대표(cover) 설정은 **지도 핀 시트만** | 탭=대표 / 탭=크게+대표 | 사용자 2026-09-15 | 2026-09-15 |
| 몰아보기 일 콜라주 = **3칸+`+N` 유지**. 탭 시 시트는 **당일 photos 전체** | 전장 그리드 / 장소 클러스터만 | 사용자 정정 2026-09-15 | 2026-09-15 |
...
| 대량 월(수천 장): cluster `id`=grid cell(멤버수 제외) + 맵 진입 시 스탬프 sync 지연 | thumb 준비될 때까지 핀 숨김만 | 2600장에서 trail만 보이던 체감 | 2026-08-02 |
...
| __DEV__ 더미: **전국 16허브 1장씩**, 오늘까지 연속일. 번들 demo 이미지. 설정에서 off | 서울·경기만 / 348장 균등 | 회고 날 칸이 매일 가득 참 · 연속 안 보임 | 2026-08-21 |
...
| **월중 추억 알림** (로컬): 월 1통·20시. 보내는 날 = 작년 같은 달 안 **새 동네(firstMonth) 날 → GPS 최다일**. 월말±2일 피함. 폴백=최근 과거 달→이번 달. 첨부 1장(iOS). 설정 스위치 없음(OS 권한만). 탭→해당 월 **몰아보기** (assetId 포커스). 테스트 버튼=설정 제목 5탭 이스터에그 | 고정 15일 / 월말과 토글 공유 / 최다일만 / 탭→지도 홈 | 사용자 2026-09-10 | 2026-09-10 |
